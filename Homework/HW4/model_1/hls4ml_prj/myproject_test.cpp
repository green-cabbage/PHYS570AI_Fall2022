...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      nnet::copy_data<float, input_t, 0, N_INPUT_1_1>(in, norm_input);
      result_t layer17_out[N_LAYER_15];

      //hls-fpga-machine-learning insert top-level-function
      unsigned short size_in1,size_out1;
      myproject(norm_input,layer17_out,size_in1,size_out1);
    ...
    input_t norm_input[N_INPUT_1_1];
    nnet::fill_zero<input_t, N_INPUT_1_1>(norm_input);
    result_t layer17_out[N_LAYER_15];
